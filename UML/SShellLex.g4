...
D_Escaped : ESCAPE . { setText(_text.substring(1)) } -> type(String), mode(SECOND_TOKEN_MODE);
...
D_Text : NOT_SPECIAL+ -> type(String), mode(SECOND_TOKEN_MODE);
D_TextRem : DOL -> type(String), mode(SECOND_TOKEN_MODE);
...
ST_Text : NOT_SPECIAL+ -> type(String);
...
A_Text : NOT_SPECIAL+ -> type(String);
