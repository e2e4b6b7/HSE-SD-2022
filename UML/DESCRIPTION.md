...
## Grep 
имплементация интерфейса `Command`, класс для команды grep. Из аргументов для `execute` собирает специальный класс `GrepArguments`.
Внутри себя использует функцию `parse`, которая парсит аргументы. В качестве библиотеки для разбора аргументов командной строки был выбран kotlinx-cli,
так как это наиболее популярная библиотека для CLI, реализованная на Kotlin.
Библиотеки, реализованные на Java менее удобны, так как не используют все возможности синтаксиса языка Kotlin. 
kotlinx-cli, использующий делегаты для описания формата CLI, делает это очень похожим на декларативный стиль, 
из-за этого, простым для написания и поддержки.

## GrepArguments
data class для представления аргументов команды `Grep`
Поля:
 - `pattern: String` -- шаблон для регулярного выражения
 - `filePath: String` -- входной файл. Может быть null, тогда аргумент берется из потока ввода команды. 
 - `wholeWord: Boolean` -- поиск только слова целиком.
 - `ignoreCase: Boolean` -- регистронезависимый поиск.
 - `numOfLinesAfterMatched: Int` -- сколько строк после совпадения надо распечатать.
