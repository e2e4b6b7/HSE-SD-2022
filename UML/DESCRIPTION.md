...
парсера,  сгенерированного при помощи библиотеки **antlr** (по грамматике в этом репозитории). Затем это дерево по
одному `Statement`’у передаётся вместе с  `VariableEnvironment` в функцию `Parser::subst` и в дереве узлы с подстановкой
...
“крайними источниками ввода-вывода”. После этого мы берем каждый из `Task` внутри `Statement`, и создаем на него
отдельный процесс, при этом в каждый из них мы форкаем текущую версию переменных (то есть внутри следующих отрезков
пайпа мы не видим появления новых переменных). Процессы мы соединяем между собой с помощью межпроцессного взаимодействия
причем последний процесс в качестве выхода имеет выход из `IO`. После этого каждый из процессов получает в качестве входа
выход предыдущего процесса, за счет этого получается пайп с выводом результата туда, куда мы хотим. При этом если `Task`
является на самом деле CommandRun, то происходит поиск команды с помощью существующего окружения команд (идем по массиву
`CommandEnvironment` и смотрим на результат операции `getCommand`, после запускаем команду), если же `Assignment`, то
изменяем значения в `VariableEnvironment`, записывая туда строчку.

После завершения для всех `Statement`, которые были разделены `;` мы снова выводим приветствие для пользователя.

## Дополнение

Если бы нужно было добавить поток ошибок, то он мог бы быть отдельным полем в классе `IO`. Если бы нам нужно было
обрабатывать код возврата, то он был бы возвращаемым значением функции `Command::eval`. 
